...
### ----------------------------------------------------------------------------------------------------- ###
### ---                                                                                               --- ###
### ---     Script to simulate survey in a East Herman - Berwick for ALL species & ALL seasons        --- ###
### ---                                                                                               --- ###
### ---                                                                                               --- ###
### ----------------------------------------------------------------------------------------------------- ###
### ----------------------------------------------------------------------------------------------------- ###
...
survey_region_label <- "EastHerman Berwick"
...
Herman_Berwick_utm <- st_transform(Herman_Berwick, st_crs(pops_dens_List$summer))
...
pops_dens_List %<>%
  map(function(x, surveyPoly = Herman_Berwick_utm){
...
if(0){
  ...
  # -- test run 
  LIDAR_results <- LIDAR_surveyRegions(swath = swath, track_gap = swath, region_polygon = Herman_Berwick_utm, 
...
Lidar_results <- sp_dens_ls %>%
  map(., function(x, strip = swath, survPoly = Herman_Berwick_utm, target = targetAnimals){
...
Lidar_results[[6]]
Lidar_results[[6]] %<>% mutate(n = round(n, digits = 0))

tm_shape(Herman_Berwick_utm) +
  tm_polygons(lwd = 2, col = "burlywood1") +
  tm_shape(Lidar_results[[6]]) +
  tm_polygons(alpha = 0.7) +
  tm_text("n", size = 0.7) +
  tm_shape(Lidar_results[[6]]) +
