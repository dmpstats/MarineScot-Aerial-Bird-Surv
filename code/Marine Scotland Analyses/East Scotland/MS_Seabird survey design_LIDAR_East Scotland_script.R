...
pops_dens_List %<>%
  map(function(x, surveyPoly = east_Scotland_utm){
...
if(0){
  ...
  # -- test run 
  LIDAR_results <- LIDAR_surveyRegions(swath = swath, track_gap = swath, region_polygon = east_Scotland_utm, 
...
Lidar_results <- sp_dens_ls %>%
  map(., function(x, strip = swath, survPoly = east_Scotland_utm, target = targetAnimals){
